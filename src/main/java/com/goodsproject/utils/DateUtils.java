...
public class DateUtils {
    ...
    public static final String DATE_FORMAT_YYYYMMDDHHMMSSSSSS = "yyyyMMddHHmmssSSSSS";
     ...
     * 判断时间A是否在时间B后
     */
    public static boolean isAfterOfTime(Date a, Date b) {
        ...
        return tomorrow.getTime() >= tomorrow1.getTime();
    }
    ...
    public static Date concatDateAndTime(Date date, Date time) {
        ...
        int month = DateUtils.getMonthByDate(date);
        int day = DateUtils.getDayByDate(date);
        int hour = DateUtils.getHourByDate(time);
        int minute = DateUtils.getMinuteByDate(time);
        Date fnanlDate = DateUtils.getDate(year, month, day, hour, minute);
        return fnanlDate;
    }
    ...
    /*
     * public static List<Date> findDates(Date dBegin, Date dEnd, Set<String>
     * stringSet) { List<Date> lDate = new ArrayList<>(); Calendar calBegin =
     * Calendar.getInstance(); calBegin.setTime(dBegin); Calendar calEnd =
     * Calendar.getInstance(); calEnd.setTime(dEnd); while
     * (dEnd.after(calBegin.getTime())) { calBegin.add(Calendar.DAY_OF_MONTH, 1); if
     * (CollectionUtils.isEmpty(stringSet)) { lDate.add(calBegin.getTime());
     * continue; } if (stringSet.contains(WeekEnum.Week.SUN.getCode()) &&
     * calBegin.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY) {
     * lDate.add(calBegin.getTime()); continue; } if
     * (stringSet.contains(WeekEnum.Week.MON.getCode()) &&
     * calBegin.get(Calendar.DAY_OF_WEEK) == Calendar.MONDAY) {
     * lDate.add(calBegin.getTime()); continue; }
     * if(stringSet.contains(WeekEnum.Week.TUE.getCode()) &&
     * calBegin.get(Calendar.DAY_OF_WEEK) == Calendar.TUESDAY){ lDate
     * .add(calBegin.getTime());continue; }
     * if(stringSet.contains(WeekEnum.Week.WED.getCode()) &&
     * calBegin.get(Calendar.DAY_OF_WEEK) == Calendar.WEDNESDAY){ lDate
     * .add(calBegin.getTime());continue; }
     * if(stringSet.contains(WeekEnum.Week.THU.getCode()) &&
     * calBegin.get(Calendar.DAY_OF_WEEK) == Calendar.THURSDAY){ lDate
     * .add(calBegin.getTime());continue; }
     * if(stringSet.contains(WeekEnum.Week.FRI.getCode()) &&
     * calBegin.get(Calendar.DAY_OF_WEEK) == Calendar.FRIDAY){ lDate
     * .add(calBegin.getTime());continue; }
     * if(stringSet.contains(WeekEnum.Week.SAT.getCode()) &&
     * calBegin.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY){ lDate
     * .add(calBegin.getTime());continue; } } return lDate; }
     */
    ...
    public static String getDateFormatDate() {
        SimpleDateFormat df = new SimpleDateFormat(DATE_FORMAT_YYYYMMDDHHMMSSSSSS);
